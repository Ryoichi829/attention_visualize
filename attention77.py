...
def self_attention(Q, K, V, scaling_factor=None):
...
# sentence = "昨日、友達と映画を見に行った。とても面白かった。"
# tokens = tokenize_with_janome(sentence)
# print("トークン化された結果:", tokens)

def get_embeddings_from_openai(text):
    """OpenAI==0.28のAPIを使用してテキストの埋め込みを取得"""
...
def main():
    ...
    if st.button("Visualize"):
        ...
        decoded_tokens = tokenize_with_janome(sentence)
        ...
        # Self-Attentionの計算
        scaling_factor = 1
        output, attention_weights = self_attention(Q, K, V, scaling_factor)
